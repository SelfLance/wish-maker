...
function PenCircle() {
  return (
    <svg viewBox="0 0 160 160" fill="none" width="160" height="160">
      <circle cx="80" cy="80" r="80" fill="linear-gradient(145deg,#F5EDD8,#EDE3C4)"/>
...
function ChainCircle() {
  return (
    <svg viewBox="0 0 160 160" fill="none" width="160" height="160">
      ...
      {/* Chain links — 2 interlocked ovals */}
      <g transform="translate(80,80) rotate(-30)">
        ...
        {/* Cover overlapping parts for interlocked look */}
        <rect x="-13" y="0" width="26" height="12" fill="#F5EDD8"/>
        <rect x="-13" y="0" width="26" height="12" fill="none" stroke="url(#chainGold)" strokeWidth="0"/>
...
export default function HowItWorksPage() {
  return (
    <div className="wm-screen" style={{ background:BG }}>
      ...
      {/* Header */}
      <div className="wm-fadeup" style={{ textAlign:"center", padding:"28px 24px 0", flexShrink:0 }}>
        <h1 style={{ fontFamily:SERIF, fontWeight:300, fontSize:"clamp(32px,4.5vw,52px)", color:TEXT }}>
          How It Works
        </h1>
      </div>
        ...
        <div className="wm-fadeup1" style={{
          ...
          {STEPS.map(({ Circle, num, title, desc }) => (
            <div key={title} style={{ display:"flex", flexDirection:"column", alignItems:"center", textAlign:"center" }}>
              ...
              <p style={{ fontFamily:SANS, fontSize:15.5, fontWeight:500, color:TEXT, marginBottom:9 }}>
                {num}. {title}
              </p>
              <p style={{ fontFamily:SANS, fontSize:14, color:TEXT2, lineHeight:1.72 }}>{desc}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ flexShrink:0, padding:"0 24px 14px" }}>
        ...
        <p style={{ fontFamily:SERIF, fontStyle:"italic", fontSize:16, color:TEXT2, textAlign:"center", marginBottom:14 }}>
        ...
        <div style={{ display:"flex", justifyContent:"center", alignItems:"center", gap:16, flexWrap:"wrap" }}>
          {["🔒 Secure SSL Encryption","⊙ Bgct Gton","✓ Norton","VERITED","VISA","AMEX"].map(t => (
            <span key={t} style={{ fontFamily:SANS, fontSize:11.5, color:TEXT3, padding:"4px 10px", border:`1px solid ${BORDER}`, borderRadius:4 }}>
              {t}
            </span>
          ))}
        </div>
